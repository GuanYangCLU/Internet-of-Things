// GUAN YANG, IT 582
// ALL COMMENTS WERE MARKED WITH "-----" AND CAPITALIZED
...
//SET BUTTON CONST HERE ----------------------------------
const int buttonPin = 13;  // SET THE BUTTON INPUT PORT #13
int i = 0;                 // THIS IS A GLOBAL COUNT TO RECORD THE BUTTON PRESSED TIMES
...
void setup()
{
  ...
  pinMode(buttonPin, INPUT); // SET THE BUTTON INPUT
...
void loop()
{
  ...
  //SET LOOP LOGIC HERE -------------------------------
  int buttonState = digitalRead(buttonPin);  // SET THE BUTTON STATE FROM BUTTON PRESS
  //int i = 0; WE CANNOT DO IT HERE BECAUSE EACH TIME THE LOOP WOULD INITIALIZE I TO 0
  if (buttonState == LOW)
  {
    i++;  // COUNT THE BUTTON PRESS TIME
  }
  
  int j = i % 6;  // MAKE IT A LOOP BY MOD METHOD
  
  switch(j)  // CONTROL DIFFERENT CONDITIONS VIA THE VARIBLE J
  {
  case 1: oneAfterAnotherNoLoop();  // *Light up all the LEDs in turn
  delay(500);
  break;

  case 2: oneOnAtATime();           // ***Turn on one LED at a time,
   delay(500);              // ***scrolling down the line
   break;
 
  case 3: oneAfterAnotherLoop();    // **Same as oneAfterAnotherNoLoop,
  delay(500);               // **but with much less typing
  break;
  
  case 4: pingPong();             // ****Light the LEDs middle to the edges
  delay(500);
  break;
  
  case 5: marquee();              // *****Chase lights like you see on signs
  delay(500);
  break;

  case 0: randomLED();            // ******Blink LEDs randomly
  delay(2000);             // END OF VOID LOOP
  break;
